...
`check_args.py` is a utility to help in running new verb experiments. Part of the key to these experiments is that the arguments associated with each position should be unbiased toward those positions prior to tuning, since that means that whatever knowledge the model ends up with about the positions of the arguments with the nonce verb must have come from the tuning and not the pre-training. For this reason, it is useful to check how well several candidate nouns adhere to this ideal. To do this, you will need the SUBTLEX corpus file `SUBTLEX-US frequency list with PoS information.xlsx`. The first time the `check_args.py` is run, it will process this file and save a reformatted version as a CSV that can be loaded faster later. The script will filter out nouns based on a `range` around a `target_freq`, and also based on whether they exist within all model tokenizers as a single token. If you set `target_freq=any`, then no filtering based on frequency of occurrence in SUBTLEX will occur.

For each candidate noun and each pair of argument positions in each sentence, the log odds ratio comparing that nouns probability in each position to each other position is obtained. For instance, if the candidate noun is 'bag,' and the sentence has a [subj] and an [obj] argument, the log odds ratio of 'bag' occurring in the position of [subj] to 'bag' occurring in the position of [obj] will be recorded. (Note that inverse ratios are not recorded, since $-log(\dfrac{x}{y}) = log(\dfrac{y}{x})$. This makes reporting them separately somewhat redundant.) The raw results are stored in `predictions.csv`. In an ideal case, this log odds ratio would be 0, which would indicate that the expected argument is not considered by the pre-trained models to be more likely in one position than it another. The difference of these measures is also reported, which gives an idea of how much more biased the models are toward putting the expected noun in one position than in another. The sum of the squared odds ratios is also calculated across all sentences for each token. The tokens associated with the lowest average sumsq and the tokens with the lowest sumsq for each model are logged for convenience, with the number of tokens logged being equal to the number of arguments requested per position (set in the tuning configuration) times the number of unique argument positions across all sentences.

You can set the following configuration options.

`dataset_loc` specifies where the CSV containing information about nouns and frequencies is located.

`target_freq` specifies the center of a range; candidate nouns must be within `range` of `target_freq` to be included as candidates. Default is `any`.
