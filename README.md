...
`cls_emb.py` fits a Support Vector Machine to embeddings of a model tokens, to determine whether they are linearly separable. Originally, this was included because we wanted to ensure that it was possible linearly separate the subject and object nouns generated by `check_args.py`. However, this is not really needed in practice, because as it turns out, it is quite trivial to linearly separate two groups of six embeddings in a 768-dimensional embedding space.
