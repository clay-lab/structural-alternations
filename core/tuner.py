...
class Tuner:
	...
	def _add_hyperparameters_to_summary_df(self, df: pd.DataFrame) -> pd.DataFrame:
		...
		# only add the optimization params to the df if we're using them
		if self.cfg.do_optimize:
			for var in self.cfg.optimize_cfg:
				# this better formats 'optimize_kwargs'
				df[f'optimize_{var}'.replace('optimize_optimize_', 'optimize_')] = str(self.cfg.optimize_cfg[var])				
	...
	def tune(self, trial: optuna.trial.Trial = None) -> None | float:
		'''
		...
		def get_optimization_metric(metrics: list | pd.DataFrame) -> float:
			'''
			Returns a summary metric during fine-tuning to use for optimization purposes,
			as specified in self.cfg.optimize_cfg.
			
			params: metrics (list | pd.DataFrame): the fine-tuning metrics. If a list is
					provided, we attempt to coerce it to a DataFrame to streamline calculations.
			'''
			# needed to return the final value using agg. Sometimes, we want the final epoch.
			# otherwise, return_which would be 'max', 'min', or any other valid pandas groupby
			# 'agg' function identifier.
			RETURN_WHICH: dict = {
			...
			# don't use metrics from the training data for optimization; these are only for record-keeping.
			targets = metrics[~metrics.dataset.str.endswith('(train)')]
			
			# the simplest case: we optimize for a scalar. Convert to a list
			# so that it matches the more general case, when we optimize for
			# an average across multiple values.
			target = self.cfg.optimize_cfg.target
			if isinstance(self.cfg.optimize_cfg.target, str):
				target = [target]
			
			targets = targets[targets.metric.isin(target)]
			
			# get the average value across all targets for each epoch so far.
			# this might seem a bit redundant, but sometimes we're optimizing
			# for maximum performance rather than final performance, so we
			# want to return the best performance achieved so far. That's why
			# we need to always consider performance at each epoch.
			targets = (
				targets
					.groupby('epoch')
					.agg({'value': 'mean'})
			...
			# determine which of the optimization metrics to return.
			# do we want the current ('last') value, or the best value
			# yet achieved ('max' or 'min')? Or some other agg function
			# of the data?
			optimization_metric = (
				targets
					.agg({
						'value': 
						RETURN_WHICH.get(
							self.cfg.optimize_cfg.return_which, 
							self.cfg.optimize_cfg.return_which
						)
					})
					.value
			)
			optimization_metric = float(optimization_metric)
			
			return optimization_metric
				
		def recursive_setattr(obj, attr, value) -> None:
			attr = attr.split('.', 1)
			if len(attr) == 1:
		...
		# if we're optimizing parameters, we need to change
		# our parameters to the specified trial suggestions.
		# this will replace the originally specified parameters
		if self.cfg.do_optimize and trial is not None:
			with open_dict(self.cfg):
				for k, v in self.cfg.optimize_cfg.params.items():
					if v.type == 'categorical':
						# we need to index into something named 'values' with a string rather than an attr,
						# since hydra reserves .values for a method of the dictconfig object
						recursive_setattr(self, k, trial.suggest_categorical(k, v['values']))
					else:
						if len(v['values']) != 2:
							raise ValueError(
						...
						# specified. The get call handles a case when kwargs
						# is missing entirely, rather than None.
						suggest_kwargs.update(v.get('suggest_kwargs', {}) or {})
					
					if v.type == 'int':
						recursive_setattr(self, k, trial.suggest_int(**suggest_kwargs))
					elif v.type == 'float':
						recursive_setattr(self, k, trial.suggest_float(**suggest_kwargs))
					else:
						raise ValueError(
							f'Can only optimize parameters of type "categorical", "int", and "float", '
							f'but {v.type!r} was provided.'
						)
		...
		if not self.tuning_data or not(isinstance(self.unfreezing,float) and np.isnan(self.unfreezing)):
			# optimization will overwrite stuff if we're running multiple optimization
			# passes, so just don't do that
			if not self.cfg.do_optimize:
		...
		# tensorboard will overwrite stuff if we're running multiple optimization
		# passes, so just don't do that			
		if not self.cfg.do_optimize:
			...
			try:
				for epoch in t:
					...
					if np.mean(dev_losses) < best_mean_loss - delta:
						...
						patience_counter 	= 0
						best_epoch 			= epoch + 1
						# don't need to do this during optimization, so save some time
						if self.save_full_model and not self.cfg.do_optimize:
					...
					metrics.append({
						...
						'metric': 'remaining patience overall', 
						'value': patience - patience_counter
					})
					...
					if not self.cfg.do_optimize:
						writer.add_scalars(
							'remaining patience', 
							{**patience_counters, 'overall': patience - patience_counter}, 
							epoch
						)
					
					postfix_kwargs = dict(
						pat=patience - patience_counter,
						avg_dev_loss=f'{np.mean(dev_losses):5.2f}',
					)
					if self.use_kl_baseline_loss or self.use_layerwise_baseline_loss:
						postfix_kwargs.update(dict(
							train_loss_only=f'{train_outputs.loss.item():5.2f}',
							train_loss_total=f'{train_loss:5.2f}',
						))
					else:
						postfix_kwargs.update(dict(train_loss=f'{train_outputs.loss.item():5.2f}'))
					
					if self.cfg.do_optimize and trial is not None:
						optimization_metric = get_optimization_metric(metrics)
						if self.cfg.optimize_cfg.target != 'loss':
							postfix_kwargs.update({self.cfg.optimize_cfg.target_label: optimization_metric})
						
						t.set_postfix(**postfix_kwargs)
						trial.report(optimization_metric, step=epoch)
		...
		# we don't want to try to save everything during optimization,
		# since it will overwrite everything and be confusing.
		if not self.cfg.do_optimize:
		...
		if self.cfg.do_optimize and trial is not None:
			return get_optimization_metric(metrics)
