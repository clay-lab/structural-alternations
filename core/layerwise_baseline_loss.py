...
# and the pretrained model's predictions as well as the l2 norm distance
# loss for each hidden layer.
import logging
...
class LayerwiseBaselineLoss(KLDivLoss):
	'''
	Calculates the loss on a dataset based on the KL divergence between the predictions
	of a model being fine-tuned and a pretrained version of that model, as well as the l2-norm distance
	for each layer. Higher KL divergences and l2-norm distances mean the model's predictions have 
	deviated more from the baseline. This penalizes deviation from the baseline model to avoid 
	catastrophic forgetting. You should add the LayerwiseBaselineLoss to another loss against 
	the fine-tuning targets during fine-tuning. Otherwise, you would just be optimizing for 
	the behavior of the original model and not striking the intended balance between performance 
	on tuning data and the original model's behavior.
	...
	def __init__(
		...
		model compared to a baseline model on a specified dataset.
			...
			params:
				...
				kl_scaleby (float)					: returned KL divergence loss is multiplied by this
													  set to 0 to remove the KL divergence loss term
				l2_scaleby (float):					: return layerwise l2-norm distance loss is multiplied by this
													  set to 0 to remove the layerwise l2-norm loss term
		...
		'''
		super(LayerwiseBaselineLoss, self).__init__(size_average, reduce, reduction)
		self.model 				= model
		...
		log.info(f'Initializing Baseline Model for LayerwiseBaselineLoss:\t\t{self.model.config.architectures[0]} ({self.model.name_or_path})')
		self.baseline_model		= AutoModelForMaskedLM.from_pretrained(self.model.name_or_path, **{**model_kwargs, 'output_hidden_states': True}).to(self.device)
		
		# we're not evaluating this
		# _ = is to prevent printing
		_ = self.baseline_model.eval()
		
		log.info(f'Initializing Baseline Tokenizer for LayerwiseBaselineLoss:\t{self.tokenizer.__class__.__name__}   ({self.tokenizer.name_or_path})')
		...
		self.kl_scaleby 		= kl_scaleby
		self.l2_scaleby			= l2_scaleby
	...
	def forward(
		...
		from self.dataset. Additionally calculates the l2-norm distance loss between
		the states of each hidden layer. Samples are randomized with each call.
		...
		total_l2_norm_distance			= torch.tensor((0.)).to(self.device)
		...
		try:
			for i, batch in enumerate(dataloader):
				...
				outputs 					= outputs.logits.index_select(-1, self.to_include)
				
				# we're not training the baseline model, so no need to get gradients for it
				with torch.no_grad():
					baseline_outputs 		= self.baseline_model(**batch_inputs)
					baseline_hidden_states 	= baseline_outputs.hidden_states
					baseline_outputs 		= baseline_outputs.logits
				...
				for i, (output, baseline_output, ex_mask_indices) in enumerate(zip(outputs, baseline_outputs, mask_indices)):
					...
					if torch.any(ex_mask_indices):
						...
						kl_div 				= super(LayerwiseBaselineLoss, self).forward(output, baseline_output)
						total_kl_div 		+= kl_div
					...
					# do we want to do this for only the masked indices? probably not I'd guess
					for hidden_state, baseline_hidden_state in zip(hidden_states, baseline_hidden_states):
						l2_dist = torch.dist(hidden_state[i], baseline_hidden_state[i], p=2)
						total_l2_norm_distance += l2_dist
						if progress_bar or return_all:
							l2_dists[-1].append(l2_dist.cpu())
				
				if progress_bar:
					dataloader.set_postfix(
						kl_div_mean=f'{np.nanmean(kl_divs):.2f}', 
						kl_div_se=f'{tuner_utils.sem(kl_divs):.2f}', 
						l2_norm_dist_mean=f'{np.mean([np.mean([v.item() for v in l]) for l in l2_dists]):.2f}',
						l2_norm_dist_se = f'{tuner_utils.sem([tuner_utils.sem([v.item() for v in l]) for l in l2_dists]):.2f}'
					)
		except KeyboardInterrupt:
			log.warning('LayerwiseBaselineLoss computation halted manually')
		...
		if return_all:
			return (
				((total_kl_div/n_included_rows) * self.kl_scaleby) + ((total_l2_norm_distance/(n_included_rows+n_not_included_rows)) * self.l2_scaleby), 
				torch.tensor(kl_divs).to(self.device), 
				torch.stack([torch.tensor(l) for l in l2_dists]).to(self.device), 
				all_mask_indices
			)
		else:
			return ((total_kl_div/n_included_rows) * self.kl_scaleby) + ((total_l2_norm_distance/(n_included_rows+n_not_included_rows)) * self.l2_scaleby)